...
import org.odpi.egeria.connectors.ibm.datastage.dataengineconnector.reports.mapping.mappers.DatabaseSchemaMapper;
 ...
 */
public class DatabaseSchemaService extends BaseService {
    private static final Logger log = LoggerFactory.getLogger(DatabaseSchemaService.class);

    public DatabaseSchemaService(DataStageReportsCache cache) {
        super(cache);
    }
     ...
     */
    DatabaseSchema getForDataStore(Identity storeIdentity) {
        if (storeIdentity == null) {
            return null;
        }
        String qualifiedName = getFullyQualifiedName(storeIdentity, null);
        if (qualifiedName != null) {
            log.debug("Constructing DatabaseSchema for data store: {}", qualifiedName);
            return DatabaseSchemaMapper.getDatabaseSchema(qualifiedName, storeIdentity.getName());
        } else {
